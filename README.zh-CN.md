...
> **当前版本：v0.2.4。** 安装与运行采用 fail-closed 策略：CodexLattice 会校验真实 Codex CLI、安装按路线固定模型/推理强度的原生角色、记录配置哈希，并在验证状态发生漂移时拒绝运行。

## 快速开始

前置要求：**Node.js >= 20**、**Codex CLI >= 0.149.0**。CodexLattice v0.2.4 已针对 Codex 0.149.1 做跨平台集成验证。
